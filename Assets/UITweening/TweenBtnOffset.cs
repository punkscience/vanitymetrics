...
namespace UnityEngine.UI
{
    ...
    public class TweenBtnOffset : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler, IPointerDownHandler, IPointerUpHandler
    {
        ...
        void IPointerDownHandler.OnPointerDown(PointerEventData eventData)
        {
            ...
            {
                if (!_Started) Start();
                TweenPos.Tween(target.gameObject, duration, rect.anchoredPosition3D, _Pos + pressed, style, method);
            }
        }
        ...
        void IPointerUpHandler.OnPointerUp(PointerEventData eventData)
        {
            if (enabled)
            {
                if (!_Started) Start();
                if (hovered)
                    TweenPos.Tween(target.gameObject, duration, rect.anchoredPosition3D, _Pos + hover, style, method);
                else
                    TweenPos.Tween(target.gameObject, duration, rect.anchoredPosition3D, _Pos, TweenMain.Style.Once, method);
            }
        }
